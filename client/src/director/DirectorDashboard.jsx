        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white dark:bg-card-dark p-6 rounded-2xl border border-slate-200 dark:border-border-dark flex items-center gap-5 shadow-sm">
            ...
            <div>
              ...
              <div className="flex items-end gap-2">
                <h3 className="text-3xl font-black dark:text-white leading-none tracking-tighter">{stats.shortlisted}</h3>
          ...
          <div className="bg-white dark:bg-card-dark p-6 rounded-2xl border border-slate-200 dark:border-border-dark flex items-center gap-5 shadow-sm">
            ...
            <div>
              ...
              <div className="flex items-end gap-2">
                <h3 className="text-3xl font-black dark:text-white leading-none tracking-tighter">{stats.pending}</h3>
          ...
          <div className="bg-white dark:bg-card-dark p-6 rounded-2xl border border-slate-200 dark:border-border-dark flex items-center gap-5 shadow-sm">
            ...
            <div>
              ...
              <div className="flex items-end gap-2">
                <h3 className="text-3xl font-black dark:text-white leading-none tracking-tighter">{stats.activeProjects}</h3>
        ...
        <section>
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-6">
              <h2 className="text-2xl font-black uppercase tracking-tight text-slate-900 dark:text-white">Recent Applications</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {recentApplications.map((app) => {
              ...
              return (
              <div key={app?._id} className="bg-white dark:bg-card-dark rounded-3xl border border-slate-200 dark:border-white/5 overflow-hidden group hover:ring-4 hover:ring-primary/10 transition-all shadow-sm">
                <div className="aspect-[3/4] relative overflow-hidden bg-slate-100 dark:bg-slate-800">
                  ...
                  <div className="absolute inset-0 bg-primary/40 backdrop-blur-[2px] opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center gap-3 px-6">
                    ...
                    <button className="w-full bg-primary text-white font-black py-3 rounded-2xl text-xs uppercase tracking-widest hover:bg-primary/90 transition-all transform translate-y-4 group-hover:translate-y-0 shadow-xl delay-75">Update Status</button>
                  </div>
                </div>
                <div className="p-6">
                  <div className="flex justify-between items-start mb-2">
                    <h4 className="font-extrabold text-lg text-slate-900 dark:text-white leading-tight truncate">{name}</h4>
                    <span className={`text-[9px] font-black uppercase px-2 py-1 rounded-md tracking-wider ${app?.status === 'shortlisted' ? 'text-emerald-500 bg-emerald-500/10' : app?.status === 'applied' ? 'text-blue-500 bg-blue-500/10' : 'text-primary bg-primary/10'}`}>
                      {app?.status || 'applied'}
                    </span>
                  </div>
                  <p className="text-xs text-slate-500 dark:text-slate-400 font-bold tracking-tight mb-4 truncate">{app?.project?.title || 'Project'}</p>
                  <div className="mt-4 flex items-center justify-between text-[10px] font-black text-slate-400 uppercase tracking-widest border-t border-slate-100 dark:border-white/5 pt-4">
                    <span className="flex items-center gap-1.5"><span className="material-symbols-outlined text-xs">location_on</span>{artist?.location || 'N/A'}</span>
