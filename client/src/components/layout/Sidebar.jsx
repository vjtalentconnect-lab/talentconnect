...
const Sidebar = ({ menuItems, userRole }) => {
    ...
    return (
        <aside className="w-72 flex-shrink-0 bg-white dark:bg-[#0f1115] border-r border-slate-200 dark:border-white/5 flex flex-col h-screen sticky top-0 transition-all duration-500">
            <div className="p-8 flex items-center gap-4 group/logo cursor-pointer">
                ...
                <div>
                    <h1 className="text-xl font-black tracking-tighter text-slate-900 dark:text-white leading-none uppercase italic">TalentConnect</h1>
                ...
                {menuItems.map((item, index) => {
                    ...
                    return (
                        <Link
                            key={index}
                            to={item.path}
                            className={`flex items-center group gap-3.5 px-5 py-3.5 rounded-2xl transition-all duration-300 relative overflow-hidden ${isActive
