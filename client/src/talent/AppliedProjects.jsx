            ...
            <div className="space-y-8 pb-12">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {[
                        ...
                        <div key={label} className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-5 flex items-center gap-4 shadow-sm">
                            <span className={`material-symbols-outlined text-2xl ${color}`}>{icon}</span>
                            <div>
                                <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">{label}</p>
                                <p className="text-2xl font-black dark:text-white italic">{count}</p>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                {filteredApps.length === 0 ? (
                    <div className="bg-white dark:bg-zinc-900 border border-dashed border-slate-200 dark:border-zinc-800 rounded-2xl p-16 text-center">
                        <span className="material-symbols-outlined text-5xl text-slate-300 mb-4 block">movie_filter</span>
                        <p className="text-slate-500 font-bold uppercase tracking-widest text-sm">No applications found</p>
                        <p className="text-slate-400 text-xs mt-2">Try a different filter or explore new projects</p>
                    </div>
                ) : (
                                ...
                                <div key={app._id}
                                    ...
                                    <div className="relative h-44 overflow-hidden">
                                        ...
                                        <div className="absolute bottom-3 left-3 z-20">
                                            <span className={`px-2.5 py-1 text-[10px] font-black rounded uppercase tracking-widest ${cfg.color}`}>
                                                {cfg.label}
                                            </span>
                                        </div>
                                        {proj?.category && (
                                            <div className="absolute top-3 right-3 z-20">
                                                <span className="px-2 py-1 bg-black/60 text-white text-[10px] font-bold rounded uppercase tracking-wider">
                                                    {proj.category}
                                                </span>
                                            </div>
                                        )}
                                    ...
                                    <div className="p-5 space-y-4">
                                        <div>
                                            <h4 className="text-lg font-bold uppercase italic tracking-tighter dark:text-white">
                                                {proj?.title || 'Project Title'}
                                            </h4>
                                            <p className="text-sm text-slate-500 flex items-center gap-1 font-bold mt-1">
                                                <span className="material-symbols-outlined text-sm text-primary">movie</span>
                                                {proj?.director?.email?.split('@')[0] || 'Production House'}
                                        ...
                                        <div className="flex justify-between items-end border-t border-slate-100 dark:border-zinc-800 pt-4">
                                            <div className="space-y-1">
                                                <p className="text-[10px] text-slate-400 uppercase font-bold tracking-widest">Applied</p>
